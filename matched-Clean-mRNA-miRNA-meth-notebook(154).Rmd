...
TNBC_meta=merge(sample_ids_primary,TNBC_meta, by="Case.ID")
...
GDCdownload(mrna_query,directory=
                      "T:\\Programming\\Data\\R\\TNBC_Confirmed_Paper\\RNA_SEQ")
...
library(finalfit)
colnames(TNBC_meta)
testing_meta= TNBC_meta
testing_meta=mrna_meta

# Specify explanatory variables of interest
explanatory <- c("Age", 
                "StageGroup","Time","Event")
table_TCGA= testing_meta %>% 
  summary_factorlist("macro",explanatory,
                      na_include=TRUE)
...
champ_directory = # set the directory 
...
library(EnhancedVolcano)
EnhancedVolcano(myDMP_volcano, x= "deltaBeta", y="adj.P.Val", lab =myDMP_volcano$gene , pCutoff = 5e-2, FCcutoff = 0.1, xlab = bquote("Methylation Difference (beta-value)"),ylab = bquote(~-Log[10] ~ italic(Adj.P)),drawConnectors = TRUE,
    widthConnectors = 0.75,colConnectors = "grey10",
  max.overlaps = 15)
  ggsave("Volcano_plot_meth.png", width = 8, height = 7, limitsize = FALSE)
...
EnhancedVolcano(
  ...
  ggsave("Volcano_plot_meth_new.png", width = 8, height = 7, limitsize = FALSE)
#For the PCA
...
setwd("T://Programming//Data//R//working directory//Final Version//Volcano_Heat")
...
colnames(activating_DEMs_118_targets)[2]="symbol"
silencing_DEMs_118_targets=silencing_DEMs_118_targets[,c("mirnaid","symbol","validated")]
colnames(silencing_DEMs_118_targets)[2]="symbol"
...
---
title: "FUSCC Mutational Analysis Cohor"
output: html_notebook
---

Mutational Analysis with CIBERSORT For FUSCC TNBC Cohort
...
dir.create("Survival\\FUSC")
setwd("Survival\\FUSC")
FUSC_meta=read_excel("Clinical_data_FUSC.xlsx",skip=1) #adapted from the article cell.com/cancer-cell/fulltext/S1535-6108(19)30096-0#supplementaryMaterial

#Exclude the patients with bad quality control, and select the patients with multi-omics
FUSC_meta <- filter(FUSC_meta,
                         DNA_QC_Failed != "TRUE",
                         Exome_Seqencing == "YES",
                         RNA_Seqencing == "YES",
                         OncoScan_Array == "YES",)
FUSC_mut_meta=FUSC_meta
```


```{r}
# The tpm expression matrix file adapted from https://figshare.com/articles/dataset/A_comprehensive_genomic_and_transcriptomic_dataset_of_triple-negative_breast_cancers/19783498
#was channeled directly to CIBERSORTx, applying the RNA-Seq settings (batch correction = on / disable quantile / perm = 100 / absolute = on)
setwd("Survival\\FUSC")
Fusc_tpm=read.table("FUSCCTNBC_Expression_RNAseqTPM_hg38.tsv",header=TRUE)
write.table(Fusc_tpm, file = "FUSCC_TPM_processed.tsv", sep = "\t", quote = FALSE, row.names = FALSE) 
...
```{r}
setwd("Survival\\FUSC")
FUSC_cibersort=read.table("FUSC_cibersort.csv",sep=',',header= T)
FUSC_cibersort=FUSC_cibersort[FUSC_cibersort$P.value<0.05,]

CIBERSORT_mutational= FUSC_cibersort[match(FUSC_mut_meta$Project_ID,FUSC_cibersort$Mixture),]
CIBERSORT_mutational=CIBERSORT_mutational[,c(1,16)]

mean_cibersort = mean(CIBERSORT_mutational[,2])
sd_cibersort = sd(CIBERSORT_mutational[,2])
  
#intiating a dataframe with identical dimension
Cutoff_mutational_matrix =  CIBERSORT_mutational 

 Cutoff_mutational_matrix[,2] = ifelse( CIBERSORT_mutational[,2] > mean_cibersort + 0.25 * sd_cibersort, "1", ifelse(CIBERSORT_mutational[,2] < mean_cibersort - 0.25 * sd_cibersort, "0", "middle"))

Cutoff_mutational_matrix= Cutoff_mutational_matrix[Cutoff_mutational_matrix$Macrophages.M1!="middle",]
 
 mutational_meta = Cutoff_mutational_matrix
 colnames(mutational_meta)[c(1,2)]= c("Sample.ID","macro")
 mutational_meta = merge(mutational_meta,FUSC_mut_meta, by.x = "Sample.ID", by.y="Project_ID")

 
High_mut_IDs=mutational_meta$Sample.ID[mutational_meta$macro=="1"]
Low_mut_IDs=mutational_meta$Sample.ID[mutational_meta$macro=="0"]
High_mut_IDs= data.frame(Tumor_Sample_Barcode=High_mut_IDs)
Low_mut_IDs= data.frame(Tumor_Sample_Barcode=Low_mut_IDs)


# Then we load the .maf files (generated by Radwan as .tsv files but simply change the extension to .maf and it works!)

 #  the following maf file is adapted from the website (nature.com/articles/s41597-022-01681-z) 

FUSC_mutations = data.table::fread("FUSCCTNBC_Mutations_Extended_hg38.maf") 

# Splitting the tmb_df into two dataframes by the merge function
High_mut_MAF = merge(High_mut_IDs,FUSC_mutations,by="Tumor_Sample_Barcode")
Low_mut_MAF = merge(Low_mut_IDs,FUSC_mutations,by="Tumor_Sample_Barcode")

unique(High_mut_MAF$Tumor_Sample_Barcode)
unique(Low_mut_MAF$Tumor_Sample_Barcode)
...
library(ggstatsplot)
#The Mutational Analysis
dir.create("Survival\\FUSC\\Plots\\Genomics",recursive=TRUE)
setwd("Survival\\FUSC\\Plots\\Genomics")

# We use read.maf function to ACTUALLY consider them maf files
High_M1 = read.maf(maf = High_mut_MAF)
Low_M1 = read.maf(maf = Low_mut_MAF)

pdf(file = "High_Low_M1_FUSCC_interactions.pdf", width = 12, height = 8)
...
# This is another graph that shows the distribution of mutations in each gene
oncoplot(maf = High_M1, titleText = "High M1", top = 10)
oncoplot(maf = Low_M1, titleText = "Low M1", top = 10)

# This is the plot for the genes pairwise mutations
somaticInteractions(maf = High_M1, pvalue = c(0.05, 0.1), top = 10, fontSize = 0.5, nShiftSymbols = 2)
somaticInteractions(maf = Low_M1, pvalue = c(0.05, 0.1), top = 10, fontSize = 0.6, nShiftSymbols = 2)
...
mutational_meta$macro[mutational_meta$macro==1]="High M1"
mutational_meta$macro[mutational_meta$macro==0]="Low M1"

#Mann-Whitney Test for Genomic Aspects
data_tmb= rbind((tmbH),(tmbL))
mutational_meta= merge(mutational_meta,data_tmb,by.x="Sample.ID", by.y="Tumor_Sample_Barcode")
mutational_meta <- arrange(mutational_meta, macro, descending = TRUE)



for (i in c(18:23,26:28,53)) {
mutational_meta[,i]= as.numeric(mutational_meta[,i])
formula <- substitute(ggbetweenstats(data = mutational_meta, x = macro, y = y_var, type = "np"),list(y_var =colnames(mutational_meta)[i]))
eval(formula)
colnames(mutational_meta)[i] = gsub("/", ".",colnames(mutational_meta)[i])
 plot_filename <-paste(colnames(mutational_meta)[i], ".png", sep="")
ggsave(plot_filename)
}
...
# A mafCompare object that provides a summary of the comparison between two cohorts (H-M1 and L-M1)
mc = mafCompare(High_M1, Low_M1, m1Name = "High M1", m2Name = "Low M1")

# Used to find differentially mutated genes. Differentially mutated genes showed contradictions with the other dataset
forestPlot(mc, pVal = 0.05)

# Capture somaticInteractions plot using dev.copy2pdf
dev.off() # the file is save as pdf in the destination assigned
...
# Reading the file
setwd("Survival\\FUSC")
CIBERSORT= read.table("FUSC_cibersort.csv",sep=',',header= T)

#Assign a cutoff values for the samples with more than 0.05 of p-value to be removed
CIBERSORT= CIBERSORT[CIBERSORT$P.value<=0.05,]

#Removing the samples that are not present
CIBERSORT= na.omit(CIBERSORT[match(FUSC_meta$Project_ID,CIBERSORT$Mixture),])
...
(Most probably will not be included because this dataset has relapse survival and the OS is not publicly available)
...
#Merging the Abundance of Immune Cells with the Survival Data of the Patients present in TNBC_meta
colnames(FUSC_meta)[c(39,37)]= c("Time","Event")
survival_cibersort_matrix = merge( x = FUSC_meta[,c(1,39,37)], y= Cutoff_matrix , by.x ="Project_ID" ,by.y="Mixture")
...
dir.create("Survival\\FUSC\\Plots\\Cells",recursive=TRUE)
  setwd("Survival\\FUSC\\Plots\\Cells")
